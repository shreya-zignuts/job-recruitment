            ...
            <div class="p-4 sm:p-8 bg-gray-300 border-solid border-5 border-black sm:rounded-lg">
                <div class="max-w-xl">
                    <section>
                        ...
                        <form method="post" action="{{ route('employer.update', ['id' => $jobs->id])}}"
                            ...
                            <div>
                                ...
                                <x-text-input class="form-control mt-1" name="company_name" id="company_name"
                                    :value="old('company_name', $jobs->company_name)" />
                                @error('company_name')
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description"
                                    class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $jobs->description) }}</textarea>
                                @error('description')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div>
                                ...
                                <x-text-input id="requirements" name="requirements" class="mt-1 block w-full"
                                    :value="old('requirements', $jobs->requirements)" />
                                @error('requirements')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    <option value="active" @if($jobs->status == 'active') selected @endif>active
                                    </option>
                                    <option value="closed" @if($jobs->status == 'closed') selected @endif>closed
                                    </option>
                                </select>
                                @error('status')
                                <div class="alert alert-danger">{{ $message }}</div>
